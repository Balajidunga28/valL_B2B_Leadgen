...
start = (datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=2)).strftime("%Y-%m-%dT%H:%M:%SZ")

url = f"https://api.render.com/v1/logs?ownerId={OWNER_ID}&resource={SERVICE_ID}&type=build&startTime={urllib.parse.quote(start)}&endTime={urllib.parse.quote(end)}&limit=100"
